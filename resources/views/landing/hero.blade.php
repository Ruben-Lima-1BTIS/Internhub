<div class="max-w-6xl mx-auto px-6 py-24 text-center">
    <h1 class="text-[3rem] font-bold text-slate-900 tracking-[-0.04em]">
        Track Your Internship Easily
    </h1>

    <p class="text-[1.15rem] text-slate-600 max-w-xl mt-6 mx-auto">
        Log hours, submit reports, and monitor progress in one clean, modern platform built for students,
        supervisors, and coordinators.
    </p>

    <div class="mt-10">
        <x-landing-page-button href="{{ route('auth.showLogin') }}">
            Get Started
        </x-landing-page-button>
    </div>
</div>
